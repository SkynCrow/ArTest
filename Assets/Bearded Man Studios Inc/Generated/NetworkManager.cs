...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] exNetworkObject = null;
		public GameObject[] GameInitNetworkObject = null;
		public GameObject[] GameLobbyNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is exNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (exNetworkObject.Length > 0 && exNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(exNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<exBehavior>();
						}
					}
			...
			else if (obj is GameInitNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (GameInitNetworkObject.Length > 0 && GameInitNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(GameInitNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<GameInitBehavior>();
						}
					}
			...
			else if (obj is GameLobbyNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (GameLobbyNetworkObject.Length > 0 && GameLobbyNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(GameLobbyNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<GameLobbyBehavior>();
						}
					}
		...
		[Obsolete("Use Instantiateex instead, its shorter and easier to type out ;)")]
		public exBehavior InstantiateexNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(exNetworkObject[index]);
			var netBehavior = go.GetComponent<exBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<exBehavior>().networkObject = (exNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateGameInit instead, its shorter and easier to type out ;)")]
		public GameInitBehavior InstantiateGameInitNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameInitNetworkObject[index]);
			var netBehavior = go.GetComponent<GameInitBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<GameInitBehavior>().networkObject = (GameInitNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateGameLobby instead, its shorter and easier to type out ;)")]
		public GameLobbyBehavior InstantiateGameLobbyNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameLobbyNetworkObject[index]);
			var netBehavior = go.GetComponent<GameLobbyBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<GameLobbyBehavior>().networkObject = (GameLobbyNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ex
		/// </summary>
		/// <returns>
		/// A local instance of exBehavior
		/// </returns>
		/// <param name="index">The index of the ex prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public exBehavior Instantiateex(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(exNetworkObject[index]);
			var netBehavior = go.GetComponent<exBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of GameInit
		/// </summary>
		/// <returns>
		/// A local instance of GameInitBehavior
		/// </returns>
		/// <param name="index">The index of the GameInit prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GameInitBehavior InstantiateGameInit(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameInitNetworkObject[index]);
			var netBehavior = go.GetComponent<GameInitBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of GameLobby
		/// </summary>
		/// <returns>
		/// A local instance of GameLobbyBehavior
		/// </returns>
		/// <param name="index">The index of the GameLobby prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GameLobbyBehavior InstantiateGameLobby(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameLobbyNetworkObject[index]);
			var netBehavior = go.GetComponent<GameLobbyBehavior>();
